  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "welcome",
      sender: "ai",
      text: "Select a topic below or type a custom prompt to generate an email draft for Abhinav.",
      timestamp: "Just now",
    },
  ]);
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="bg-[var(--card)] text-[var(--card-foreground)] border border-[var(--border)] rounded-[var(--radius)] p-3.5 sm:p-4 md:p-5 shadow-xs hover:border-[var(--brand-accent)]/30 transition-all duration-200">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-center">
            {/* Left Column: Direct Contact Points (Minimal without emojis) */}
            ...
            {/* Right Column: Minimal AI Email & Chat Interface */}
            <div className="lg:col-span-7">
              ...
              <div className="flex items-center justify-between pb-2 mb-2 border-b border-[var(--border)]/60">
                ...
                <button
                  ...
                  className="px-2 py-0.5 rounded text-[11px] font-medium text-[var(--muted-foreground)] hover:text-[var(--foreground)] hover:bg-[var(--accent)] transition-colors cursor-pointer"
              ...
              <div className="flex flex-col space-y-2">
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none select-none">
                  {QUICK_PROMPTS.map((item) => (
                    <button
                      ...
                      className="whitespace-nowrap px-2.5 py-1 rounded-full text-[10.5px] font-medium bg-[var(--background)] hover:bg-[var(--accent)] text-[var(--foreground)] border border-[var(--border)] hover:border-[var(--brand-accent)]/50 transition-all cursor-pointer shadow-2xs shrink-0"
                    >
                      {item.label}
                    </button>
                  ))}
                ...
                <div className="h-[155px] sm:h-[175px] overflow-y-auto space-y-2 pr-1 text-xs rounded-[var(--radius)] bg-[var(--background)]/60 border border-[var(--border)]/70 p-2.5">
                  {messages.map((msg) => (
                    ...
                    >
                      <div
                        className={`max-w-[92%] rounded-lg px-2.5 py-1.5 leading-relaxed text-[11px] ${
                          msg.sender === "user"
                        ...
                        {msg.draft && (
                          <div className="mt-2 pt-2 border-t border-[var(--border)]/70 space-y-1.5 text-left">
                            <div className="bg-[var(--accent)]/50 rounded px-2 py-1 border border-[var(--border)]/60">
                              <span className="text-[9px] font-bold text-[var(--brand-accent)] uppercase tracking-wider block">
                                Subject
                              </span>
                              <span className="text-[10.5px] font-semibold text-[var(--foreground)] block truncate">
                                {msg.draft.subject}
                              </span>
                            </div>

                            <div className="bg-[var(--background)] rounded p-2 text-[10px] font-mono text-[var(--foreground)] whitespace-pre-wrap max-h-24 overflow-y-auto border border-[var(--border)]/40 leading-normal">
                              {msg.draft.body}
                            </div>

                            {/* Actions: Open in Gmail & Copy Draft */}
                            <div className="flex items-center gap-2 pt-1">
                              <a
                                href={getGmailUrl(msg.draft)}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-1.5 px-3 py-1 rounded-[var(--radius)] bg-[var(--brand-accent)] hover:bg-[#96c19c] text-[#0E1613] text-[11px] font-bold transition-all shadow-xs cursor-pointer"
                              ...
                              <button
                                ...
                                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-[var(--radius)] bg-[var(--accent)] hover:bg-[var(--border)] text-[var(--foreground)] text-[10.5px] font-semibold border border-[var(--border)] transition-all cursor-pointer"
                ...
                >
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-1 px-3 py-1.5 rounded-[var(--radius)] bg-[var(--brand-accent)] text-[#0E1613] font-bold text-xs hover:bg-[#96c19c] active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-2xs cursor-pointer shrink-0"
                  >
                    <span>Send</span>
                    <svg
                      stroke="currentColor"
